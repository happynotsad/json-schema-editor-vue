<template>
  <div class="json-schema-editor">
    <a-row class="row" :gutter="10">
      ...
      <a-col>
        <a-textarea :disabled="isArray || isObject" @blur="changeMockValue" :value="mockText" :rows="2"
          placeholder="mock"></a-textarea>
      </a-col>
      <a-col>
        <a-input-number v-model.number="min" type="number" v-if="!isBoolean" style="width:100%" @blur="changeMin"
          placeholder="minLength" />
      </a-col>
      <a-col>
        <a-input-number v-model="max" aria-disabled="isBoolean"  style="width:100%" @blur="changeMax"
          placeholder="minLength" />
      </a-col>
...
export default {
  ...
  computed: {
    ...
    isBoolean() {
      return this.pickValue.type === 'boolean'
    },
    checked() {
  ...
  methods: {
    ...
    changeMockValue(e) {
      ...
      if (!value || value === '') {
        this.pickValue.mock = null
        return
      }
      var arr = value.split('\n')
      // this.pickValue.mock = arr.map(item => item);
      // console.log(this.pickValue)
      this.$set(this.pickValue, 'mock', arr.map(item => item))
      console.log(this.pickValue)
    },
    changeMin(e){
      this.$set(this.pickValue, 'min', Number(e.target.value))
    },
    changeMax(e){
      this.$set(this.pickValue, 'max', e.target.value)
    },
